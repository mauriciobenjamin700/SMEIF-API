...
def test_uc_classes_add_event_success(db_session, mock_ClassEventRequest):
...
def test_uc_classes_add_event_fail_conflict(
        db_session, 
    ...
    with raises(HTTPException) as exception:
        uc.add_event(request)

    assert exception.value.status_code == 409
    assert exception.value.detail == ERROR_CLASSES_EVENTS_ADD_CONFLICT


def test_uc_classes_add_event_fail_class_not_found(
        db_session, 
        mock_ClassEventRequest
):
...
def test_uc_classes_add_event_fail_discipline_not_found_discipline(
        db_session, 
        mock_ClassEventRequest
):
...
def test_uc_classes_add_event_fail_discipline_not_found_disciplines(
        db_session, 
        mock_ClassEventRequest
):
...
def test_uc_classes_add_event_fail_teacher_not_found(
        db_session, 
        mock_ClassEventRequest
):
