...
def test_uc_classes_get_event_success(
    db_session,
    ...
    response = uc.get_event(class_event_id=mock_class_event_on_db.id)
...
def test_uc_classes_get_event_not_found(
    db_session
):
    ...
    with raises(HTTPException) as exception:
        uc.get_event(class_event_id="not_found")
