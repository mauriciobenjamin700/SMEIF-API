...
def test_controller_teacher_get_success_no_disciplines_and_no_classes(db_session, mock_teacher_on_db):
    controller = TeacherController(db_session)

    response = controller.get(mock_teacher_on_db.cpf)
...
def test_controller_teacher_get_success_with_disciplines_and_classes(
    db_session, 
    mock_teacher_on_db, 
...
def test_controller_teacher_get_with_classes_and_not_disciplines(
db_session, 
mock_teacher_on_db,
mock_class_teacher_on_db
):
...
def test_controller_teacher_get_with_disciplines_and_not_classes(
    db_session, 
    mock_teacher_on_db,
    mock_teacher_discipline_on_db
):
...
def test_controller_teacher_get_invalid_teacher(db_session, mock_user_on_db):
    controller = TeacherController(db_session)

    with raises(HTTPException) as exception:

        controller.get(mock_user_on_db.cpf)
