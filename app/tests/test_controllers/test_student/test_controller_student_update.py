...
def test_StudentController_update_success(
    db_session: Session,
    mock_student_on_db,
    mock_ChildRequest_update
):
    # Arrange
    student_controller = StudentController(db_session)
    request = mock_ChildRequest_update

    # Act
    response = student_controller.update(
        request=request
    )
...
def test_StudentController_update_fail_empty(
    db_session: Session,
    mock_ChildRequest_update
):
